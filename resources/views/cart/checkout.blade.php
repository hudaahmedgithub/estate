            ...
            @foreach($cartItems as $cartItem)
                ...
                <tr>
                    <td class="cart_product">
                        <a href="{{url('/product_details')}}/{{$cartItem->id}}"><img src="{{url('dist/images',$cartItem->options->img)}}" alt="" width="200px"></a>
                    </td>
                    ...
                    <td class="cart_quantity">
                        ...
                        <div class="cart_quantity_button">
                            ...
                            <input type="number" size="2" value="{{$cartItem->qty}}" name="qty" id="upCart<?php echo $count;?>" autocomplete="off" style="text-align:center; max-width:50px; "  MIN="1" MAX="30" required>
                            <input type="submit" class="btn btn-primary" value="Update" styel="margin:5px" required>
                        </div>
                        </form>
                    </td>
                    <td class="cart_total">
                            ...
                            <form action="{{url('formvalidate')}}" method="post">
                                ...
                                <div class="row">
                                    <div class="form-group col-md-6">
                                        ...
                                        <input id="firstname" type="text" name="fullname"required placeholder="Display Name"  value="{{ old('firstname') }}" class="form-control">
                                        <br>
                                        <span style="color:red">{{ $errors->first('fullname') }}</span>
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label for="lastname" class="form-label">State Name</label>
                                        <input id="lastname" type="text" name="state"required placeholder="State Name" value="{{ old('state') }}" class="form-control">
                                        <br>
                                        <span style="color:red">{{ $errors->first('state') }}</span>
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label for="lastname" class="form-label">Pincode</label>
                                        <input id="lastname" type="text" name="pincode" placeholder="Pincode" required value="{{ old('pincode') }}" class="form-control">
                                        <br>
                                        <span style="color:red">{{ $errors->first('pincode') }}</span>
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label for="lastname" class="form-label">City Name</label>
                                        <input id="lastname" type="text" name="city"required placeholder="City Name" value="{{ old('city') }}" class="form-control">
                                        <br>
